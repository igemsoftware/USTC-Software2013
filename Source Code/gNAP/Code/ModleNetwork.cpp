    ...
    for(i1=0;i1!=ny+1;++i1)
    {
        for(j1=0;j1!=nx+1;++j1)
        {
            if(i1<ny&&j1<nx)
            {
                ...
                {
                    case 1:
                        b[i1][j1]=(double)(rand()%71)/100.0+0.5;
                        break;
                    case -1:
                        b[i1][j1]=-(double)(rand()%71)/100.0-0.5;
                        break;
            ...
            else
            {
                ...
                {
                if(rand()%100<abs(m1))
                    k1=m1>0?(double)(rand()%71)/100.0+0.5:-(double)(rand()%101)/150.0-0.5;
                else if
                    (rand()%10)k1=0;
                else
                    k1=m1<0?(double)(rand()%71)/100.0+0.5:-(double)(rand()%101)/150.0-0.5;
...
{
    double *a,*b,*c,*d,*e,*f,sum(0),AbsValue(1),Score(0);
        double sco[6];
        ...
        fi1.open("data/Score");
        for(i=0;i<5;++i)
            fi1<<i+0.5<<' '<<sco[i]<<endl;
            fi1.close();
        for(i=0;i<GENEAM;++i)
            for(j=0;j<TFScale;++j)
            MaxMa[i][j]=ReguMatrix[i][j]*((rand()%71)/100.0+0.5);
        for(i=0;i<ny;++i)
            a[i]=b[i]=e[i]=INITIALVALUE;
        AbsValue=10;
        j=0;
        ...
        d[ny]=f[ny]=c[ny];
        ofstream igemSfw;
        igemSfw.open("data/EachGeneChange");
        if(!igemSfw)
            exit(0);
        j=0;
        cou=0;
